...
import { getLostItems, deleteItem } from '../utils/storage';

const LostScreen = ({ user }) => {
  ...
  const loadItems = async () => {
    ...
    setItems(lostItems.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)));
  };
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          style: 'destructive',
          onPress: async () => {
            const result = await deleteItem(itemId, 'lost');
            if (result.success) {
              loadItems();
            } else {
